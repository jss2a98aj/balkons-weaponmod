...
public abstract class PhysHelper {
    ...
    public static AdvancedExplosion createStandardExplosion(World world, Entity entity, double d, double d1, double d2,
            float size) {
    ...
    public static AdvancedExplosion createAdvancedExplosion(World world, Entity entity, double d, double d1, double d2,
            float size, boolean destroyBlocks, boolean spawnSmallParticles, boolean spawnBigParticles) {
        AdvancedExplosion explosion = new AdvancedExplosion(world, entity, d, d1, d2, size);
        explosion.doEntityExplosion();
        if (destroyBlocks) {
            explosion.doBlockExplosion();
    ...
    public static AdvancedExplosion createAdvancedExplosion(World world, Entity entity, DamageSource damagesource,
            double d, double d1, double d2, float size, boolean destroyBlocks, boolean spawnSmallParticles,
    ...
    public static AdvancedExplosion createAdvancedExplosion(World world, Entity entity, double d, double d1, double d2,
            float size, boolean destroyBlocks, boolean spawnParticles) {
        AdvancedExplosion explosion = new AdvancedExplosion(world, entity, d, d1, d2, size);
        explosion.doEntityExplosion();
        if (destroyBlocks) {
            explosion.doBlockExplosion();
    ...
    private static void sendExplosion(World world, AdvancedExplosion explosion, boolean smallparts, boolean bigparts) {
        if (world instanceof WorldServer && !world.isRemote) {
            ...
            BalkonsWeaponMod.instance.messagePipeline.sendToAllAround(
                    msg,
                    new TargetPoint(
    ...
    public static void knockBack(EntityLivingBase entityliving, EntityLivingBase attacker, float knockback) {
        ...
        for (dz = attacker.posZ - entityliving.posZ; dx * dx + dz * dz
                < 1E-4D; dz = (Math.random() - Math.random()) * 0.01D) {
