...
public abstract class PlayerWeaponData {
    ...
    public static void initPlayerWeaponData(EntityPlayer player) {
        ...
        try {
            ...
            BalkonsWeaponMod.modLog
                    .warn("DataWatcher ID conflict for " + playername + " @ " + WARHAMMER_LAST_SMASH_TICKS);
        } catch (NullPointerException e) {} finally {
    ...
    private static void unavailableError(EntityPlayer player, int id) {
        BalkonsWeaponMod.modLog.error(
                "DataWatcher ID " + id + " for " + getPlayerName(player) + " unavailable, trying to reinitialize");
    ...
    /*
     * private static byte getByte(int i, int pos) { pos <<= 2; return (byte) ((i >>> pos) & 0xFF); } private static int
     * setByte(int i, int pos, byte b) { //TODO return i; }
     */
}
