...
public class DispenseCannonBall extends BehaviorDefaultDispenseItem {
    ...
    public ItemStack dispenseStack(IBlockSource blocksource, ItemStack itemstack) {
        ...
        normalDispense = false;
        TileEntity tileentity = blocksource.getWorld().getTileEntity(
                MathHelper.floor_double(blocksource.getXInt()),
                MathHelper.floor_double(blocksource.getYInt()),
                MathHelper.floor_double(blocksource.getZInt()));
        ...
        EntityCannonBall entitycannonball = new EntityCannonBall(
    ...
    protected void playDispenseSound(IBlockSource blocksource) {
        ...
        blocksource.getWorld().playSoundEffect(
                blocksource.getX(),
                blocksource.getY(),
                blocksource.getZ(),
                "random.explode",
                8.0F,
                1.0F / (rand.nextFloat() * 0.8F + 0.9F));
        blocksource.getWorld().playSoundEffect(
                blocksource.getX(),
                blocksource.getY(),
                blocksource.getZ(),
    ...
    protected void spawnDispenseParticles(IBlockSource blocksource, EnumFacing face) {
        ...
        if (!normalDispense) {
            IPosition pos = BlockDispenser.func_149939_a(blocksource);
            blocksource.getWorld().spawnParticle(
                    "flame",
