...
public class ItemBlowgunDart extends WMItem {
    ...
    public IIcon getIconFromDamage(int damage) {
        return (damage >= 0 && damage < DartType.dartTypes.length && DartType.dartTypes[damage] != null)
                ? DartType.dartTypes[damage].itemIcon
                : itemIcon;
    }
    ...
    public void addInformation(ItemStack itemstack, EntityPlayer entityplayer, @SuppressWarnings("rawtypes") List list,
            boolean flag) {
        ...
        /*
         * Map map = potion.func_111186_k(); if (map != null && map.size() > 0) { Iterator iterator1 =
         * map.entrySet().iterator(); while (iterator1.hasNext()) { Entry entry = (Entry) iterator1.next();
         * AttributeModifier attributemodifier = (AttributeModifier) entry.getValue(); AttributeModifier
         * attributemodifier1 = new AttributeModifier(attributemodifier.func_111166_b(),
         * potion.func_111183_a(potioneffect.getAmplifier(), attributemodifier), attributemodifier.func_111169_c());
