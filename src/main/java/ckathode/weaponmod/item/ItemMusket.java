...
public class ItemMusket extends ItemShooter {
    ...
    public ItemMusket(String id, MeleeComponent meleecomponent, Item bayonetitem) {
        ...
        if (meleecomponent.meleeSpecs != MeleeSpecs.NONE && meleecomponent.weaponMaterial != null) {
            bayonetDurability = meleecomponent.meleeSpecs.durabilityBase
    ...
    public boolean hitEntity(ItemStack itemstack, EntityLivingBase entityliving, EntityLivingBase attacker) {
        ...
        if (attacker instanceof EntityPlayer && !((EntityPlayer) attacker).capabilities.isCreativeMode) {
            ...
            if (bayonetdamage > bayonetDurability) {
                /*
                 * if (deltadamage > 0) { int m = EnchantmentHelper.getEnchantmentLevel(Enchantment.unbreaking.effectId,
                 * itemstack); int k = 0; for (int l = 0; j > 0 && l < deltadamage; ++l) { if
                 * (EnchantmentDurability.negateDamage(itemstack, m, entityplayer.getRNG())) { ++k; } } deltadamage -=
                 * k; if (deltadamage <= 0) { deltadamage = 0; } }
                 */
