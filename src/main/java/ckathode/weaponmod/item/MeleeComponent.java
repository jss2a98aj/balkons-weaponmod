...
public class MeleeComponent extends AbstractWeaponComponent {
    ...
    public float getBlockDamage(ItemStack itemstack, Block block) {
        ...
        return material != Material.plants && material != Material.vine
                ...
                && material != Material.gourd ? 1.0F : meleeSpecs.blockDamage;
    }
    ...
    public void addItemAttributeModifiers(Multimap<String, AttributeModifier> multimap) {
        ...
        if (dmg > 0F || meleeSpecs.damageMult > 0F) {
            multimap.put(
                    ...
                    new AttributeModifier(weapon.getUUID(), "Weapon modifier", dmg, 0));
        }
        multimap.put(
                WeaponModAttributes.WEAPON_KNOCKBACK.getAttributeUnlocalizedName(),
                new AttributeModifier(
                        weapon.getUUID(),
                        "Weapon knockback modifier",
                        meleeSpecs.getKnockBack(weaponMaterial) - 0.4F,
                        0));
        multimap.put(
                WeaponModAttributes.ATTACK_SPEED.getAttributeUnlocalizedName(),
                new AttributeModifier(weapon.getUUID(), "Weapon attack speed modifier", -meleeSpecs.attackDelay, 0));
        if (this instanceof IExtendedReachItem) {
            try {
                multimap.put(
                        WeaponModAttributes.WEAPON_REACH.getAttributeUnlocalizedName(),
                        new AttributeModifier(
                                weapon.getUUID(),
                                "Weapon reach modifier",
                                ((IExtendedReachItem) this).getExtendedReach(null, null, null) - 3F,
                                0));
    ...
    public static enum MeleeSpecs {
        ...
        private MeleeSpecs(int durbase, float durmult, float dmgbase, float dmgmult, float blockdmg, float knockback,
                int dmgfromentity, int dmgfromblock, int stacksize, int attackdelay) {
