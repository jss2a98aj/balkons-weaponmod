...
public abstract class RangedComponent extends AbstractWeaponComponent {
    ...
    public void addItemAttributeModifiers(Multimap<String, AttributeModifier> multimap) {
        multimap.put(
                WeaponModAttributes.RELOAD_TIME.getAttributeUnlocalizedName(),
                new AttributeModifier(weapon.getUUID(), "Weapon reloadtime modifier", rangedSpecs.getReloadTime(), 0));
    }
    ...
    public boolean hasAmmoAndConsume(ItemStack itemstack, World world, EntityPlayer entityplayer) {
        return entityplayer.capabilities.isCreativeMode
                ...
                || entityplayer.inventory.consumeInventoryItem(getAmmoItem());
    }

    public boolean hasAmmo(ItemStack itemstack, World world, EntityPlayer entityplayer) {
        return entityplayer.capabilities.isCreativeMode
                || EnchantmentHelper.getEnchantmentLevel(Enchantment.infinity.effectId, itemstack) > 0
                || entityplayer.inventory.hasItem(getAmmoItem());
    }
        ...
        public int getReloadTime() {
            if (reloadTime < 0 && BalkonsWeaponMod.instance != null) {
                ...
                BalkonsWeaponMod.modLog
                        .debug("Found reaload time " + reloadTime + " for " + reloadTimeTag + " @" + this);
            }
            return reloadTime;
        }
