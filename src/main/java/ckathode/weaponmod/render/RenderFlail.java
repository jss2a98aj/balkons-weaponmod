...
public class RenderFlail extends Render {
    ...
    public void renderFlail(EntityFlail entityarrow, double d, double d1, double d2, float f, float f1) {
        ...
        if (entityarrow.shootingEntity instanceof EntityLivingBase) {
            ...
            vec3d.rotateAroundX(
                    (-(entityarrow.shootingEntity.prevRotationPitch
                            + (entityarrow.shootingEntity.rotationPitch - entityarrow.shootingEntity.prevRotationPitch)
                                    * f1)
                            * 3.141593F) / 180F);
            vec3d.rotateAroundY(
                    (-(entityarrow.shootingEntity.prevRotationYaw
                            + (entityarrow.shootingEntity.rotationYaw - entityarrow.shootingEntity.prevRotationYaw)
                                    * f1)
                            * 3.141593F) / 180F);
            vec3d.rotateAroundY(f23 * 0.5F);
            vec3d.rotateAroundX(-f23 * 0.7F);
            double d7 = entityarrow.shootingEntity.prevPosX
                    + (entityarrow.shootingEntity.posX - entityarrow.shootingEntity.prevPosX) * f1
                    + vec3d.xCoord;
            double d8 = entityarrow.shootingEntity.prevPosY
                    + (entityarrow.shootingEntity.posY - entityarrow.shootingEntity.prevPosY) * f1
                    + vec3d.yCoord;
            double d9 = entityarrow.shootingEntity.prevPosZ
                    + (entityarrow.shootingEntity.posZ - entityarrow.shootingEntity.prevPosZ) * f1
                    + vec3d.zCoord;
            if (renderManager.options.thirdPersonView > 0) {
                float f21 = ((((EntityLivingBase) entityarrow.shootingEntity).prevRenderYawOffset
                        + (((EntityLivingBase) entityarrow.shootingEntity).renderYawOffset
                                - ((EntityLivingBase) entityarrow.shootingEntity).prevRenderYawOffset) * f1)
                        * 3.141593F) / 180F;
                double d4 = MathHelper.sin(f21);
                double d6 = MathHelper.cos(f21);
                d7 = (entityarrow.shootingEntity.prevPosX
                        + (entityarrow.shootingEntity.posX - entityarrow.shootingEntity.prevPosX) * f1) - d6 * 0.35D
                        - d4 * 0.85D;
                d8 = (entityarrow.shootingEntity.prevPosY
                        + (entityarrow.shootingEntity.posY - entityarrow.shootingEntity.prevPosY) * f1) - 0.45D;
                d9 = ((entityarrow.shootingEntity.prevPosZ
                        + (entityarrow.shootingEntity.posZ - entityarrow.shootingEntity.prevPosZ) * f1) - d4 * 0.35D)
                        + d6 * 0.85D;
            }
